let { SeuModel } = require('../models/tarefasModel')
...
const getAll = (req,res) => {
    db.query('SELECT * FROM Tarefas', (err, data)=>{
        if(err){
            ...
            res.status(500).json({error: 'Erro interno de servidor ao buscar tarefas'})
        }else{
...
const addTarefa = (req, res) =>{
    try {
        ...
        if(!descricao){
            return res.status(400).json({error: 'É necessário enviar uma descrição.'})
        }
        db.query('INSERT INTO tarefas (descricao, concluido) VALUES ($1, false)', [descricao], (err)=>{
            if(err){
                console.error('Erro ao adicionar tarefas:', err);
                console.log(req.body)
                res.status(500).json({error: 'Erro interno de servidor ao tentar adicionar tarefas'})
            }else{
        ...
        res.status(500).json({error:'Erro interno de servidor ao tentar adicionar tarefa.'})
    }
}
    ...
    try {
        ...
        db.query('SELECT * FROM tarefas WHERE id = $1', [tarefaId], (err, rows) => {
            if (err) {
                ...
                return res.status(500).json({ error: 'Erro interno de servidor ao verificar tarefa' });
            }
            ...
            db.query(`UPDATE tarefas SET descricao = $1, concluido = $2 WHERE id = $3`, [descricao, concluido, tarefaId], (err, result) => {
                if (err) {
                    console.error('Erro ao atualizar tarefa:', err.message);
                    return res.status(500).json({ error: 'Erro interno de servidor ao atualizar tarefa', err });
                } else {
        ...
        res.status(500).json({ error: 'Erro interno de servidor ao tentar Atualizar tarefa.', err })
    }
}
...
const deleteTarefa = (req,res) =>{
    try {
        ...
        db.query('SELECT * FROM tarefas WHERE id = $1', [tarefaId], (err, row) => {
            ...
            db.query('DELETE FROM tarefas WHERE id =$1', [tarefaId], (err)=>{
                if(err){
                    ...
                    res.status(500).json({error: 'Erro interno de servidor ao tentar deletar tarefa.', err})
                }else{
        ...
        res.status(500).json({error:'Erro interno de servidor ao tentar deletar tarefa.'})
    }
}
